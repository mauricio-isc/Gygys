...
public interface PagoRepository extends JpaRepository<Pago, Long> {
    ...
    List<Object[]> countAndSumByMetodoPagoAndFechaPagoAfter(@Param("fechaInicio") LocalDateTime fechaInicio);

    @Query("SELECT p FROM Pago p WHERE p.membresia.id = :membresia ORDER BY p.fechaPago DESC")
    List<Pago> findByMembresiaOrderByFechaPagoDesc(@Param("membresia") Membresia membresia);

    @Query("SELECT p FROM Pago p WHERE p.membresia.miembro = :miembro ORDER BY p.fechaPago DESC")
    List<Pago> findByMembresiaMiembroOrderByFechaPagoDesc(@Param("miembro") Miembro miembro);
}
